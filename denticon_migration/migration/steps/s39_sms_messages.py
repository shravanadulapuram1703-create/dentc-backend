...
def run(conn, maps: dict) -> dict:
    ...
    # reload. Leaf table.
    cur = conn.cursor()
    cur.execute("TRUNCATE TABLE sms_messages RESTART IDENTITY")
    conn.commit()

    skipped = 0
    buf = BulkBuffer(
        conn, "sms_messages", COLS,
        conflict="ON CONFLICT DO NOTHING",
        flush_every=20000, page_size=2000, label="sms_messages",
    )
    ...
    buf.flush()
    print(f"  [s39] sms_messages: {buf.inserted} inserted, {skipped} skipped")
    return {}
