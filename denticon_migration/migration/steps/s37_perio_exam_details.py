...
def run(conn, maps: dict) -> dict:
    ...
    skipped = 0
    buf = BulkBuffer(
        conn, "perio_exam_details", COLS,
        conflict="ON CONFLICT DO NOTHING",
        flush_every=20000, page_size=1000, label="perio_exam_details",
    )
    ...
    buf.flush()
    print(f"  [s37] perio_exam_details: {buf.inserted} inserted, {skipped} skipped")
    return {}
